...
export const useSimulation = () => {
  const [simulationState, setSimulationState] = useState<SimulationState>({
    ...
    activeModelId: 'tiny-llama', virtualUsers: [], activityLog: []
  });
  ...
  const tick = useCallback(() => {
    ...
    const currentModel = MODELS[state.activeModelId];
    ...
    newUsers = newUsers.map(user => {
        ...
        if (u.state === UserState.IDLE) {
            ...
            if (--u.timer <= 0) {
                ...
                newRequests.push({ id: reqId, progress: 0, totalTokens: prompt.tokens, parallelShards: currentModel.tpSize, targetNodeId, color: u.color });
    ...
    // 3. Process Requests
    const baseSpeed = currentModel.tokensPerSec / 60;
    ...
    newUsers = newUsers.map(u => {
        if (u.state === UserState.WAITING && u.currentRequestId && finished.has(u.currentRequestId)) {
            const cost = (finished.get(u.currentRequestId).totalTokens / 1000) * currentModel.costPer1kTokens;
    ...
    const newNodes = state.nodes.map(node => {
        ...
        const load = currentModel.tpSize > 1 ? activeReqs : newRequests.filter(r => r.targetNodeId === node.id).length;
        const util = node.gpuUtil + (Math.min(100, load * 15) - node.gpuUtil) * 0.1;
        return { 
            ...node, gpuUtil: util, vramUtil: node.vramUtil + (Math.min(100, currentModel.vramPerGpu + load * (currentModel.id === 'llama-405b' ? 1.5 : 0.5)) - node.vramUtil) * 0.1,
            temp: 30 + util * 0.4 + Math.random(), status: util > 2 ? NodeStatus.COMPUTING : NodeStatus.IDLE, activeTokens: load > 0 ? Math.floor(util * (currentModel.id === 'llama-405b' ? 0.2 : 5)) : 0
        };
    });
    ...
    const newMetric: MetricPoint = {
        timestamp: newTime, totalThroughput: throughput, avgLatency: activeReqs > 0 ? (currentModel.id === 'llama-405b' ? 80 : 10) + activeReqs : 0,
        clusterUtilization: newNodes.reduce((acc, n) => acc + n.gpuUtil, 0) / newNodes.length,
        queueDepth: activeReqs, activeUsers: newUsers.length, estimatedCostPerHour: (throughput / 1000) * currentModel.costPer1kTokens * 3600,
